...
public class ControllerLoadFeatureModel extends AbstractAction {
    ...
    public ControllerLoadFeatureModel(ModelPLEDGE model, ViewPLEDGE view) {
        this.model = model;
        this.view = view;
    }
    ...
    public void actionPerformed(ActionEvent e) {
        File selected = view.displayFileChooser(ViewPLEDGE.FileType.FEATURE_MODEL);
        if (selected != null) {
            ...
            final Thread t = new Thread(new Runnable() {
                ...
                public void run() {
                    try {
                        model.loadFeatureModel(featureModelPath, featureModelPath.toLowerCase().endsWith("xml") ? ModelPLEDGE.FeatureModelFormat.SPLOT : ModelPLEDGE.FeatureModelFormat.DIMACS);
                    } catch (Exception ex) {
