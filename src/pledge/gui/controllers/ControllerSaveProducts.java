 ...
 */
public class ControllerSaveProducts extends AbstractAction {

    private ModelPLEDGE model;
    private ViewPLEDGE view;

    public ControllerSaveProducts(ModelPLEDGE model, ViewPLEDGE view) {
        this.model = model;
        this.view = view;
    }
    ...
    public void actionPerformed(ActionEvent e) {
        File selected = view.displayFileSaver();
        if (selected != null) {
            final String savePath = selected.getPath();
            final Thread t = new Thread(new Runnable() {

                @Override
                public void run() {
                    try {
                        model.saveProducts(savePath);
                    } catch (Exception ex) {
